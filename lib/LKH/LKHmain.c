...
void Clean_Mem() {
    ...
    if(Depot) free(Depot);
    if(FirstActive) free(FirstActive);
    if(LastActive) free(LastActive);
    if(FirstConstraint) free(FirstConstraint);
    if(FirstNode) free(FirstNode);
    if(FirstSegment) free(FirstSegment);
    if(FirstSSegment) free(FirstSSegment);
}
