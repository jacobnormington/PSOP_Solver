...
class Active_Path {
    ...
    public:
        Active_Path();
        Active_Path(int num, int arr_size);
        ...
        bool pop_back(Active_Allocator* Allocator, bool out_dated);
        void set_threadID(int id, int total_id);
        void reconstruct_path(Active_Path& partial_path, Active_Allocator* Allocator);
        void clear(Active_Allocator* Allocator);
        void print_curpath();
};
